  ...
  return (
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        {/* Tabs */}
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-ms p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab) => (
              <button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-ms transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'} `}>
          ...
          {dataArray.find((item) => item.label === currentTab).value.map((user) => (
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className='flex-1'>
                <p className='font-medium text-skate-700'>{user.full_name}</p>
                <p className='text-skate-500'>{user.username}</p>
                <p className='text-sm text-skate-500'>{user.bio.slice(0, 30)}...</p>
